...
source_count = 0
rr_count = 0
PP.pp VitalStatistics.oclcs
SourceRecord.where(oclc_resolved:{"$in":VitalStatistics.oclcs}).no_timeout.each do |src|
  source_count += 1
...
puts "VS sources: #{source_count}"
puts "Initial RR count: #{rr_count}"

deprecate_count = 0
rr_count = 0

SourceRecord.where(series:"VitalStatistics", 
