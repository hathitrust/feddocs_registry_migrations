...
# Parse enumchrons for Census of Manufactures registry records 
#
#
CM = CensusOfManufactures
source_count = 0
rr_count = 0
SourceRecord.where(oclc_resolved:{"$in":CM.oclcs}).no_timeout.each do |src|
  source_count += 1
  src.series = src.series
  src.save
  RegistryRecord.where(source_record_ids:src.source_id, 
                       series:{"$ne":"Census Of Manufactures"}).no_timeout.each do |r|
    r.series ||= []
    r.series << "Census Of Manufactures"
    r.series.uniq! 
    rr_count += 1
    r.save
  end
end
puts "CM sources: #{source_count}"
puts "Initial RR count: #{rr_count}"
deprecate_count = 0
rr_count = 0

SourceRecord.where(series:"CensusOfManufactures", 
