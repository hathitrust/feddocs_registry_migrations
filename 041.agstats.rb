...
# Parse enumchrons for AgStat registry records 
...
# but didn't add bad ones.
=end 
RegistryRecord.where(series:"Agricultural Statistics",
                     enumchron_display:/C\. \d/,
                     deprecated_timestamp:{"$exists":0}).no_timeout.each do |reg|
  reg.deprecate('Improved Agricultural Statistics enum/chron parsing.')
  deprecate_count += 1
end

# Re-extract all the Source Records
SourceRecord.where(series: "AgriculturalStatistics",
  ...
  src.enum_chrons.each do | ec | 
    if regrec = RegistryRecord.where(series:"Agricultural Statistics", 
                                     deprecated_timestamp:{"$exists":0}, 
                                     enumchron_display:ec).first
      regrec.add_source(src)
    else
      regrec = RegistryRecord.new([src.source_id], ec, "Improved Ag Stats enum/chron parsing.")
      rr_count += 1
    end
    regrec.series = "Agricultural Statistics"
    regrec.save
  end
  src.save
