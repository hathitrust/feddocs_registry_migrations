...
# Parse enumchrons for Current Population Report registry records 
#
#
CPR = CurrentPopulationReport
source_count = 0
rr_count = 0
SourceRecord.where(oclc_resolved:{"$in":CPR.oclcs}).no_timeout.each do |src|
  source_count += 1
  src.series = src.series
  src.save
  RegistryRecord.where(source_record_ids:src.source_id, 
                       series:{"$ne":"Current Population Report"}).no_timeout.each do |r|
    r.series ||= []
    r.series << "Current Population Report"
    r.series.uniq! 
    rr_count += 1
    r.save
  end
end
puts "CPR sources: #{source_count}"
puts "Initial RR count: #{rr_count}"
deprecate_count = 0
rr_count = 0

SourceRecord.where(series:"CurrentPopulationReport", 
