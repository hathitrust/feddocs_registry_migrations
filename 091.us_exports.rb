...
# Parse enumchrons for U S Exports registry records 
#
#
Exp = USExports
source_count = 0
rr_count = 0
SourceRecord.where(oclc_resolved:{"$in":Exp.oclcs}).no_timeout.each do |src|
  source_count += 1
  src.series = src.series
  src.save
  RegistryRecord.where(source_record_ids:src.source_id, 
                       series:{"$ne":"U S Exports"}).no_timeout.each do |r|
    r.series ||= []
    r.series << "U S Exports"
    r.series.uniq! 
    rr_count += 1
    r.save
  end
end
puts "US Export sources: #{source_count}"
puts "Initial RR count: #{rr_count}"
deprecate_count = 0
rr_count = 0

SourceRecord.where(series:"USExports", 
