...
include Registry
# Parse enumchrons for SEC News Digest registry records 
#
#
digest_ocns = [2650712,8198194,40650268,426253974]

source_count = 0
rr_count = 0
SourceRecord.where(oclc_resolved:{"$in":digest_ocns}).no_timeout.each do |src|
  source_count += 1
  src.series = src.series
  src.save
  RegistryRecord.where(source_record_ids:src.source_id, 
                       series:{"$ne":'SEC News Digest'},
...
puts "SEC News Digest sources: #{source_count}"
puts "Initial RR count: #{rr_count}"
deprecate_count = 0
rr_count = 0

SourceRecord.where(series:'SEC News Digest', 
