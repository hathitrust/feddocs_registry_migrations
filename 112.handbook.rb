...
include Registry
# Parse enumchrons for Handbook of NA Indians registry records 
#
ocns = [13240086, 568690282, 622355838, 670012899, 679862056]
...
SourceRecord.where(oclc_resolved:{"$in":ocns}).no_timeout.each do |src|
  ...
  RegistryRecord.where(source_record_ids:src.source_id, 
                       series:{"$ne":'Handbook of North American Indians'},
...
puts "Handbook sources: #{source_count}"
puts "Initial RR count: #{rr_count}"
deprecate_count = 0
rr_count = 0

SourceRecord.where(series:'Handbook of North American Indians', 
