...
# Parse enumchrons for Calendar of Business registry records 
#
#
COB = CalendarOfBusiness
source_count = 0
rr_count = 0
SourceRecord.where(oclc_resolved:{"$in":COB.oclcs}).no_timeout.each do |src|
  source_count += 1
...
puts "Calendar Of Business sources: #{source_count}"
puts "Initial RR count: #{rr_count}"
deprecate_count = 0
rr_count = 0

SourceRecord.where(series:"CalendarOfBusiness", 
