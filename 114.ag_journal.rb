...
include Registry
# Parse enumchrons for Journal of Agricultural Research registry records 
#
...
SourceRecord.where(oclc_resolved:{"$in":ocns}).no_timeout.each do |src|
  ...
  RegistryRecord.where(source_record_ids:src.source_id, 
                       series:{"$ne":'Journal of Agricultural Research'},
...
puts "Journal of Ag Research sources: #{source_count}"
puts "Initial RR count: #{rr_count}"
deprecate_count = 0
rr_count = 0

SourceRecord.where(series:'Journal of Agricultural Research', 
